...
const ProductModal = ({ isOpen, setIsOpen, setCart }) => {
  ...
  const [alignment, setAlignment] = useState(
    sizes?.length > 0 ? sizes[0].price : price[0]
  );
  ...
  useEffect(() => {
    ...
    if (isOpen.product?.id) {
      const selected_price =
        isOpen.product?.sizes?.length > 0
          ? isOpen.product?.sizes[0].price
          : isOpen.product?.price[0];
      const item_details = {
        ...isOpen.product,
        selected_price,
        sauce:
          isOpen.product?.sauces?.length > 0 ? isOpen.product?.sauces[0] : null,
  ...
  const handleId = (itemDetails) => {
    ...
    const _id = `${itemDetails?.name?.split(' ').join('_')}_F:${
      itemDetails.flavor || null
    }_A:${itemDetails.add_on ? itemDetails.add_on : null}_P:${
      itemDetails.selected_price
    }_S:${itemDetails.sauce}`;
    return _id;
  };
          ...
          <Grid container>
            ...
            <Grid item xs={12} sx={{ p: 2, pt: 1 }}>
              <Typography variant='h5' sx={{ mb: 2 }}>
                {is_best_seller && (
                  <span className='best-seller row'>Best Seller</span>
                )}
                <strong>{name}</strong>
              </Typography>
              ...
              <Box fullWidth>
                ...
                <ToggleButtonGroup
                  ...
                  color='primary'
                  disabled={
                    (sizes?.length > 0 ? [...sizes] : [...price]).length <= 0
                  }
                >
                  {/* ₱ */}
                  {(sizes?.length > 0 ? [...sizes] : [...price]).map(
                    (price, idx) => {
                      return (
                        <ToggleButton
                          key={idx}
                        ...
                        >
                          ...
                          >
                            ...
                            <Typography variant='h6'>
                              {sizes?.length > 0 ? price.price : price}.00
                            </Typography>
                          </div>
                        </ToggleButton>
                      );
